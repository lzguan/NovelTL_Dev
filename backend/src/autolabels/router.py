...
async def read_autolabel_by_id(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail=f"No autolabel with id {auto_label_id} found.") from e
    return autolabel
...
@router.post("/auto-labels", response_model=list[schemas.AutoLabelMeta])
async def create_autolabels(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unknown error occured: {str(e)}") from e
