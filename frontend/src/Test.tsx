...
function collectAbsoluteLabels(
	manager: SegmentManager<DemoStyle, DemoLabel>,
): ManagedLabel<DemoStyle, DemoLabel>[] {
...
function measureSelectionRects(
	...
	for (const segment of manager.getSegments()) {
		...
		const touchesCollapsedBoundary =
			isCollapsed && start >= segmentStart && start <= segmentEnd;
...
function Test() {
	...
	const clearIntersectingLabels = useCallback(() => {
		...
		const labelIds = collectAbsoluteLabels(manager)
			.filter(
				(label) => label.interval.start < selectedEnd && label.interval.end > selectedStart,
			)
			.map((label) => label.id);
	...
	return (
		...
		>
			<div
				ref={editorRootRef}
				style={{ maxWidth: 1080, margin: "0 auto", display: "grid", gap: "1rem" }}
			>
				<section>
					<h1 style={{ margin: 0, fontSize: "1.4rem" }}>DynamicLabeledText Editor POC</h1>
					<p style={{ margin: "0.5rem 0 0", color: "#475569" }}>
						Click, drag, type, backspace, paste, copy, cut, and add labels from the
						current selection.
					</p>
				</section>
				...
				>
					<InfoCard title="Editor State">
						...
						<StateRow
							label="selected text"
							value={selectedText ? JSON.stringify(selectedText) : "(none)"}
						/>
