    ...
    <main>
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-10">
        <header className="flex flex-col gap-3 text-left">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-base text-muted-foreground">
              Search source works, expand their linked novels inline, or open the dedicated
              source-work page.
            </p>
          </div>
        </header>
        ...
        <div className="flex flex-col gap-3 rounded-xl border bg-card p-4">
          ...
          <StaticRouteInput
            toHref={(s: string) => routeTo.view.sourceworks({ search: s })}
            defaultValue={search}
          />
          <p className="text-sm text-muted-foreground">Expand a work to see its related novels.</p>
        </div>
        ...
        {!loading && !error && sourceWorkData.length > 0 ? (
          <SourceWorkList data={sourceWorkData} />
        ) : null}
      </section>
    </main>
  );
