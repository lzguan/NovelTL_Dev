...
export function buildUIManager(
	initialText: string,
	initialLabels: ManagedLabel<MyStyle, StyledLabel<MyStyle>>[],
): UIManager {
		...
		segmentManager.batch(() => {
			labelIds.forEach((labelId) => {
				...
				segmentManager.updateLabel(labelId, {
					style: [{ ...label.style[0] }, { ...label.style[1], visible }],
	...
	const toggleActiveStatus = (labelIds: ProvisionalId[], active: boolean) => {
		segmentManager.batch(() => {
			labelIds.forEach((labelId) => {
				const label = segmentManager.getLabel(labelId);
				segmentManager.updateLabel(labelId, {
					style: [{ ...label.style[0] }, { ...label.style[1], active }],
					interval: label.interval,
				});
			});
		});
				...
				segmentManager.updateLabel(labelId, {
					style: [
						{ ...label.style[0] },
						{
							...label.style[1],
							cursorStatus:
								label.style[1].cursorStatus === "clicked" ? "clicked" : hover,
						},
					],
					interval: label.interval,
				});
