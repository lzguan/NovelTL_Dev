...
export function productReducer<const Styles extends readonly Style[]>(
    ...
    return (styles) => {
        const result = reducers.map((reducer, index) => {
            const coordinateStyles = styles.map(
                (style) => style[index],
            ) as Styles[typeof index][];
...
export function makeUnderlineStyleReducer(defaultUnderline: boolean = false): StyleReducer<UnderlineStyle> {
    return (styles) => ({
        underline: styles.length === 0
            ? defaultUnderline
            : styles.some((style) => style.underline),
    });
}

export function makeBoldStyleReducer(defaultBold: boolean = false): StyleReducer<BoldStyle> {
    return (styles) => ({
        bold: styles.length === 0
            ? defaultBold
            : styles.some((style) => style.bold),
    });
}
...
export function priorityReducer<S extends Style>(reducer : StyleReducer<S>): StyleReducer<PriorityStyle<S>> {
    return (styles) => {
        if (styles.length === 0) {
            return {
                priority: Infinity,
