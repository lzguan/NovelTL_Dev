...
type CallbackParams<
	...
	ID extends string,
	E extends React.SyntheticEvent<HTMLDivElement>,
> = {
	event: E;
	manager: SegmentManager<S, L, ID>;
	caret: Caret;
};

type EditorCallbacks<S extends Style, L extends StyledLabel<S>, ID extends string> = {
	onPointerDown?: ({
		event,
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.PointerEvent<HTMLDivElement>>) => void;
	onPointerMove?: ({
		event,
		manager,
		caret,
	...
	onClick?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.MouseEvent<HTMLDivElement>>) => void;
	onDoubleClick?: ({
		event,
		manager,
		caret,
	...
	onKeyDown?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.KeyboardEvent<HTMLDivElement>>) => void;
	onKeyUp?: ({
		event,
		manager,
		caret,
	...
	onCopy?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.ClipboardEvent<HTMLDivElement>>) => void;
	onCut?: ({
		event,
		manager,
		caret,
	...
	onCompositionStart?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.CompositionEvent<HTMLDivElement>>) => void;
	onCompositionUpdate?: ({
		event,
		manager,
		caret,
	...
	onBeforeInput?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.InputEvent<HTMLDivElement>>) => void;
	onInput?: ({
		event,
		manager,
		caret,
	...
	onFocus?: ({
		...
		manager,
		caret,
	}: CallbackParams<S, L, ID, React.FocusEvent<HTMLDivElement>>) => void;
	onBlur?: ({
		event,
		manager,
		caret,
...
function DynamicLabeledText<S extends Style, L extends StyledLabel<S>, ID extends string>({
	caret,
	manager,
	render,
	containerStyle,
	...
	caret: Caret;
	manager: SegmentManager<S, L, ID>;
	render: EditorRenderer<S, L>;
	containerStyle?: React.CSSProperties;
	overlayStyle?: React.CSSProperties;
	caretOverlayStyle?: React.CSSProperties;
} & EditorCallbacks<S, L, ID>) {
		...
		>
			...
			<div
				...
				onCompositionStart={(e) =>
					callbacks.onCompositionStart?.({ event: e, manager, caret })
				}
				onCompositionUpdate={(e) =>
					callbacks.onCompositionUpdate?.({ event: e, manager, caret })
				}
