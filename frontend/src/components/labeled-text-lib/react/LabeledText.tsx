    ...
    return (
        <div>
            <div ref={containerRef} style={{position: "relative", ...props.containerStyle}}>
                <div ref={overlayRef} style={{ position: "absolute", inset: 0, pointerEvents: "none", ...props.overlayStyle }}>
                    {
                        Overlay ? segments.map((segment) => (
                            <Overlay key={segment.start} segment={segment} containerRef={containerRef} overlayRef={overlayRef} />
                        )) : null
                    }
                </div>
