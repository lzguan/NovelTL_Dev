...
export type ReducedSegment<S extends Style> = Segment<S, Label<S>> & { [reducedBrand] : true };

export function asReducedSegment<S extends Style>(segment: Segment<S, Label<S>>): ReducedSegment<S> {
    return segment as ReducedSegment<S>;
}

export type FullReducedSegment<S extends Style> = ReducedSegment<S> & { [fullReducedBrand] : true };

export function asFullReducedSegment<S extends Style>(segment: Segment<S, Label<S>>): FullReducedSegment<S> {
 ...
 */
export type Segmenter<S extends Style, L extends Label<S>> = (text : string, labels : L[]) => Segment<S, L>[];

export type ReducingSegmenter<S extends Style, L extends Label<S>> = (text : string, labels : L[]) => ReducedSegment<S>[];

export type FullReducingSegmenter<S extends Style, L extends Label<S>> = (text : string, labels : L[]) => FullReducedSegment<S>[];
